...
Unlike CLI `nanobot onboard` (initial setup), this is the IN-CHAT onboarding
...
from nanobot.models.role_card import BUILTIN_BOTS, RoleCardDomain
...
"""
    ...
    def _get_team_intro_message(self) -> str:
        ...
        default_team_name = theme.name.value.replace("_", " ").title() if theme else "Crew"
...
Need more details on any particular crew member? Just ask!"""
    ...
    def _build_team_table(self) -> Table:
        ...
        theme = self.theme_manager.get_current_theme()
        header_emoji = theme.leader.emoji if theme else "⚓"
...
"""
    ...
    @classmethod
    def from_dict(cls, data: dict, workspace_path: Path, theme_manager: ThemeManager) -> "ChatOnboarding":
