...
- nanobot heartbeat start [--bot BOT_NAME]
- nanobot heartbeat stop [--bot BOT_NAME]
- nanobot heartbeat status [--bot BOT_NAME]
- nanobot heartbeat trigger [--bot BOT_NAME] [--reason REASON]
- nanobot heartbeat logs [--bot BOT_NAME] [--limit N]
"""
...
def _print_bot_status(bot, status):
    ...
    if history:
        ...
        for entry in history[-5:]:
            ...
            if event == "tick":
                result = f"✅ {details.get('checks_run', 0)} checks" if details.get("success") else f"❌ Failed"
            elif event == "check":
