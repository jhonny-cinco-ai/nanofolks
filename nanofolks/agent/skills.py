    ...
    def build_skills_summary(self, show_all: bool = True) -> str:
        ...
        if pending_skills:
            ...
            for s in pending_skills:
                name = escape_xml(s["name"])
                lines.append("  <skill available=\"false\" verified=\"pending\" requires=\"security_approval\">")
                lines.append(f"    <name>{name} (PENDING APPROVAL)</name>")
                lines.append("    <description>This skill has been installed but needs security verification before use. Run security scan and approve if safe.</description>")
