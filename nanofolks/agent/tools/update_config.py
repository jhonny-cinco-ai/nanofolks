            ...
            "providers": {
                ...
                "moonshot": {
                    "fields": {
                        "apiKey": {
                            "type": "string",
                            "help": "Get from https://platform.moonshot.cn/",
                        },
                        "apiBase": {"type": "url", "default": None},
                    },
                    "models": [
            ...
            "channels": {
                "telegram": {
                    ...
                    "fields": {
                        ...
                        "token": {
                            "type": "string",
                            "help": "Bot token from @BotFather (format: 123456:ABC-DEF...). Steps: 1) Message @BotFather 2) Type /newbot 3) Copy the token",
                        },
                        "allowFrom": {
                            "type": "array",
                            "item_type": "string",
                            "help": "Allowed user IDs or usernames (comma-separated, e.g.: user1, user2, 123456789)",
                        },
                    },
                },
                "discord": {
                ...
                "whatsapp": {
                    ...
                    "fields": {
                        ...
                        "bridgeUrl": {
                            "type": "url",
                            "default": "",
                            "help": "Leave empty for auto-config (recommended)",
                        },
                        "bridgeToken": {
                            "type": "string",
                            "help": "Shared secret for bridge auth (required)",
                            "required": True,
                        },
                        "allowFrom": {
                            "type": "array",
                            "item_type": "string",
                            "help": "Allowed phone numbers (comma-separated, e.g.: +1234567890, +0987654321)",
                        },
                    },
                },
                "slack": {
                ...
                "email": {
                    ...
                    "fields": {
                        ...
                        "consentGranted": {
                            "type": "boolean",
                            "default": False,
                            "help": "Explicit permission to access mailbox",
                        },
                        ...
                        "allowFrom": {
                            "type": "array",
                            "item_type": "string",
                            "help": "Allowed sender email addresses (comma-separated, e.g.: user@example.com, admin@domain.com)",
                        },
                    },
                },
            },
        ...
        "tools": {
            ...
            "fields": {
                "restrictToWorkspace": {
                    "type": "boolean",
                    "default": False,
                    "help": "Sandbox: restrict file access to workspace only",
                },
                "evolutionary": {
                    "type": "boolean",
                    "default": True,
                    "help": "Allow bot to self-improve (see docs)",
                },
                "allowedPaths": {
                    "type": "array",
                    "item_type": "path",
                    "help": "Paths bot can access when evolutionary=true",
                },
                "protectedPaths": {
                    "type": "array",
                    "item_type": "path",
                    "default": ["~/.nanofolks/config.json"],
                },
        ...
        "routing": {
            ...
            "fields": {
                "enabled": {
                    "type": "boolean",
                    "default": True,
                    "help": "Enable smart routing (recommended)",
                },
            },
            "tiers": {
            ...
            "advanced": {
                ...
                "clientClassifier.minConfidence": {
                    "type": "float",
                    "default": 0.85,
                    "min": 0.0,
                    "max": 1.0,
                    "help": "Confidence threshold for client-side classification",
                },
                "llmClassifier.model": {
                    "type": "string",
                    "default": "gpt-4o-mini",
                    "help": "Model for LLM-assisted classification",
                },
                "sticky.contextWindow": {
                    "type": "integer",
                    "default": 5,
                    "min": 1,
                    "max": 20,
                    "help": "Context window for sticky routing",
                },
                "sticky.downgradeConfidence": {
                    "type": "float",
                    "default": 0.9,
                    "min": 0.0,
                    "max": 1.0,
                    "help": "Confidence threshold for tier downgrade",
                },
                "autoCalibration.enabled": {
                    "type": "boolean",
                    "default": True,
                    "help": "Enable automatic routing calibration",
                },
            },
        },
    }
...
"""
    ...
    def _validate_value(self, value: Any, schema: dict, path: str) -> Any:
        ...
        try:
            if field_type == "string":
                str_value = str(value)
                # Skip pattern validation for empty strings and keyring markers
                if str_value or "pattern" not in schema:
                    # Check pattern if specified (skip for keyring markers)
                    if "pattern" in schema and str_value and str_value != KEYRING_MARKER:
                        import re
            ...
            raise ConfigUpdateError(
                f"Invalid value type. Expected {field_type}, got {type(value).__name__}"
            )
    ...
    def _update_config_value(
        self, config: Config, parts: list[str], value: Any, operation: str
    ) -> bool:
        """
    ...
    def validate_model_for_routing(self, model: str) -> dict:
        ...
        if "/" in model:
            ...
            if "claude" in model_lower:
                ...
                "gpt" in model_lower or model_lower.startswith("o1") or model_lower.startswith("o3")
            ):
        ...
        if not provider_name:
            ...
            result["warning"] = (
                f"Could not determine provider for model '{model}'. Ensure you have the correct provider configured."
            )
            return result
        ...
        if provider_config and provider_config.api_key:
            ...
            configured = [
                name
                for name in self.SCHEMA["providers"]["providers"].keys()
            ...
            if configured:
                ...
                if is_gateway:
                    result["warning"] = f"Model '{model}' requires {provider_name.title()} gateway."
                    result["suggestion"] = (
                    ...
                    result["suggestion"] = (
                        "Run 'nanofolks onboard' to configure your first provider."
                    )
    ...
    def get_config_summary(self) -> dict:
        ...
        summary["has_required_config"] = any(p["has_key"] for p in summary["providers"].values())
