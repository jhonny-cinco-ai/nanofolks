...
from nanofolks.config.schema import SlackConfig
...
class SlackChannel(BaseChannel):
    ...
    _MD_TO_SLACK = (
        (r'(?m)(^ |\s)\*\*\*(.+?)\*\*\*(\s|$)', r'\1*_\2_*\3'),  # ***text*** -> *_text_*
        (r'(?m)(^ |\s)___(.+?)___(\s|$)', r'\1*_\2_*\3'),  # ___text___ -> *_text_*
        (r'(?m)(^ |\s)\*\*(.+?)\*\*(\s|$)', r'\1*\2*\3'),  # **text** -> *text*
        (r'(?m)(^ |\s)__(.+?)__(\s|$)', r'\1*\2*\3'),  # __text__ -> *text*
        (r'(?m)(^ |\s)\*(.+?)\*(\s|$)', r'\1_\2_\3'),  # *text* -> _text_
        (r'(?m)(^ |\s)~~(.+?)~~(\s|$)', r'\1~\2~\3'),  # ~~text~~ -> ~text~
        (r'(^ |\s)\[(.+?)\]\((https?:.+?)(?:\s".*?")?\)(\s|$)', r'\1<\3>\4'),  # [text](url) -> <url>
        (r'!\[.+?\]\((http.+?)(?:\s".*?")?\)', r'<\1>'),  # ![alt](url) -> <url>
    )
