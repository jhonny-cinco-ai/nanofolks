    ...
    with console.status("[cyan]Loading memory status...[/cyan]", spinner="dots"):
        try:
            ...
            try:
                ...
                memory_store.get_learning_stats()
            ...
            for entity_type, count in entity_summary.items():
                console.print("[bold yellow]Entity Summary:[/bold yellow]")
                console.print(f"  {entity_type}: {count}")

            # Display counts
            console.print("[bold green]✓ Import Complete![/bold cyan]")
        ...
        try:
            ...
            if summary_nodes:
                ...
                if show_details and node.get('summary'):
                    ...
                    console.print("[bold cyan]No summary details available")
...
def memory_import(
    ...
    if not confirm:
        if not typer.confirm("This will replace ALL memory data. Continue?"):
