"""Data models for team routines engine (legacy heartbeat).

This module defines the core data structures for per-bot team routines,
...
class CheckPriority(Enum):
    """Priority levels for team routines checks."""
    CRITICAL = "critical"    # Run immediately, alert on failure
    HIGH = "high"           # Run promptly, log failures
    NORMAL = "normal"       # Run during normal team routines
    LOW = "low"             # Run when resources available


class CheckStatus(Enum):
    """Status of a team routines check execution."""
    PENDING = "pending"
    RUNNING = "running"
    SUCCESS = "success"
    FAILED = "failed"
...
class CheckDefinition:
    """Definition of a single team routines check.
...
@dataclass
class CheckResult:
    """Result of executing a team routines check.
...
@dataclass
class TeamRoutinesConfig:
    """Configuration for a bot's team routines.

    Attributes:
        bot_name: Name of the bot this config belongs to
        interval_s: Seconds between team routines ticks (default: 3600 = 60 min)
        max_execution_time_s: Maximum time for entire tick
        enabled: Whether team routines are enabled
...
@dataclass
class TeamRoutinesTick:
    """A single team routines execution.
...
@dataclass
class TeamRoutinesHistory:
    """History of team routines executions for a bot.
    ...
    """
    bot_name: str
    ticks: List[TeamRoutinesTick] = field(default_factory=list)
    ...
    def add_tick(self, tick: TeamRoutinesTick) -> None:
